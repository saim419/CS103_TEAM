...
class transactions():
    ...
    def update_transaction(self, item_number=None, amount=None, category=None, date=None, description=None):
        ...
        #print(sql + " (item_number,))")
        #print(updates)
        #return self.cursor.execute("UPDATE transactions SET amount = (?) WHERE rowid=(?)",(item_number,))
        #return self.cursor.execute("UPDATE transactions SET amount = 300.0, category = 'Entertainment' WHERE rowid=(?) ", 
        #(item_number,))
        return self.cursor.execute(sql, (item_number,))
